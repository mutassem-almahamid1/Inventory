...
using Services.Features.Orders.Queries.GetAllOrders;
using Services.Features.Orders.Queries.GetOrderById;
using Services.Features.Orders.Queries.GetOrdersBySupplier;
...
public class OrdersController(ISender sender) : ControllerBase
{
    ...
    public async Task<ActionResult> GetBySupplier(Guid supplierId)
    {
        var result = await sender.Send(new GetOrdersBySupplierQuery(supplierId));
        if (result.IsSuccess)
            return Ok(result.Value);
